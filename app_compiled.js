...
document.addEventListener('click', function (e) {
  ...
  var inLightboxView = modal.className === 'modal' ? true : false; // In other words, if the modal's class name is 'modal no-display', it means the lightbox is hidden.
...
function UpdatePage(result) {
  ...
  var htmlInSlideshow = result.reduce(function (str, item, index) {
    return str + "<div class=\"slide\">\n        <img\n          src=".concat(item.webformatURL.replace('_640', '_960'), "\n          srcset=\"").concat(item.webformatURL.replace('_640', '_340'), " 340w, ").concat(item.webformatURL, " 640w, ").concat(item.webformatURL.replace('_640', '_960'), " 960w\"\n          sizes=\"80vw\"\n          alt=\"").concat(item.tags, "\"\n          class=").concat(slideImgClass, ">\n        <div class=\"overlay\">\n          tags: ").concat(item.tags, "\n        </div>\n      </div>");
  }, '');
  ...
  if (easterEggShown) {
    ...
    document.querySelector('.easter-egg').innerHTML = '<button aria-label="Load more images">Load More</button>';
  }
}
...
function conductSearch(term) {
  ...
  currentSearchTerm = term; // Update prompt message.
